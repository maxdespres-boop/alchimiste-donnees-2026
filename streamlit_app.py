...
def harmoniser_formats_alc(row):
    ...
    qty = row['LineQty']
    # AJUSTEMENT : On divise par 2 QUE si c'est du SAP (2026+) et que ça finit par 12.
    # Les données 2025 (courtier) sont déjà en format caisse.
...
try:
    if df_raw_all is not None:
        ...
        if page == "Alchimiste":
            ...
            k3.metric("CAISSES 2025 (YTD)", f"{total_eq_2025_ytd:,.1f}", delta=f"{total_eq_2026 - total_eq_2025_ytd:,.1f}")
            k4.metric("VENTES $ 2025 (YTD)", f"{total_val_2025_ytd:,.2f} $", delta=f"{total_val_2026 - total_val_2025_ytd:,.2f} $")

            # 3. Graphiques YoY (Volume et Dollars)
            st.header("📈 Comparaison Performance (YTD)")
            yoy_vol = df_alc.pivot_table(index='Mois_Nom', columns='Année', values='CAISSE EQ', aggfunc='sum').fillna(0)
            
            # Pour l'onglet financier, on s'assure que 2025 est filtré YTD pour matcher l'app
            df_fin_ytd = pd.concat([df_2026, df_2025_ytd])
            yoy_val = df_fin_ytd.pivot_table(index='Mois_Nom', columns='Année', values='LineTotal', aggfunc='sum').fillna(0)
            
            t1, t2 = st.tabs(["Volume (Caisses)", "Chiffre d'Affaires ($)"])
            with t1: st.plotly_chart(px.line(yoy_vol.reset_index(), x='Mois_Nom', y=yoy_vol.columns, markers=True), use_container_width=True)
            with t2: st.plotly_chart(px.line(yoy_val.reset_index(), x='Mois_Nom', y=yoy_val.columns, markers=True), use_container_width=True)
            ...
            # Export Excel (YTD Dollars inclus pour balancement)
            output = io.BytesIO()
            with pd.ExcelWriter(output, engine='xlsxwriter') as writer:
                week_summary.to_excel(writer, sheet_name='Focus_Semaine', index=False)
                yoy_vol.to_excel(writer, sheet_name='Volume_YOY_Caisse')
                yoy_val.to_excel(writer, sheet_name='Ventes_YOY_Dollars_YTD')
                sku_data.to_excel(writer, sheet_name='Detail_Produits_Periode')
